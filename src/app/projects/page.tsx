...
export default function ProjectsPage() {
  ...
  const renderProjectCard = (project: Project) => (
    <div key={project.id} className="bg-[#121212] border border-white/10 rounded-2xl overflow-hidden shadow-xl hover:border-pink-500/40 transition-all duration-300 flex flex-col h-full group">
      ...
      <div className="p-5 flex flex-col flex-grow justify-between">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5 mb-4">
            {(() => {
              const stack: any = project.tech_stack;
              let items: string[] = [];

              if (Array.isArray(stack)) {
                items = stack;
              } else if (typeof stack === 'string') {
                items = stack.replace(/[\[\]"']/g, '').split(/[,\.]+/);
              }
              return items.filter(Boolean).map((tech: string, index: number) => (
                <span
                  key={index}
                  className="bg-pink-500/10 text-pink-400 border border-pink-500/10 text-[10px] font-medium px-2.5 py-1 rounded-full shadow-sm whitespace-nowrap block"
                >
                  {tech.trim()}
                </span>
              ));
          ...
          <div className="flex items-center justify-between pt-3 border-t border-white/5 text-xs font-semibold">
            {project.github_url ? (
              <a href={project.github_url} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-white flex items-center gap-1.5 transition-colors">
            ...
            {project.demo_url ? (
              <a href={project.demo_url} target="_blank" rel="noopener noreferrer" className="text-pink-400 hover:text-pink-300 hover:underline flex items-center gap-1 transition-colors">
                Live Preview <i className="bi bi-arrow-up-right text-[10px]"></i>
              </a>
            ) : (
              <span className="text-gray-600 cursor-not-allowed">No Demo</span>
            )}
          </div>
        </div>
      </div>
