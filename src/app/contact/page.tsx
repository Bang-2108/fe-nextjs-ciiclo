...
export default function ContactPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus({
        success: false,
        msg: 'Unable to send your message at the moment. Please try again later.',
      });
    } finally {
  ...
  const handleViewCV = () => {
    if (profile?.cv_path) {
      ...
      alert('The resume is currently being updated.');
    }
  };

  const emailData = profile?.email || 'zoanthibang@gmail.com';
  const githubLink = 'https://github.com/Bang-2108/';
  const linkedinLink =
    'https://www.linkedin.com/in/zo%C3%A3n-th%E1%BB%8B-b%C4%83ng-v135798642/';
    ...
    <div className="min-h-screen bg-black text-white p-6 pt-28 flex flex-col items-center">
      ...
      <div className="max-w-5xl w-full grid grid-cols-1 md:grid-cols-12 gap-6 items-stretch">
        <div className="md:col-span-7 bg-[#111827]/60 border border-white/5 rounded-3xl p-6 md:p-8 shadow-xl flex flex-col h-full">
          ...
          {status && (
            <div
              className={`p-3.5 rounded-xl text-center text-xs font-semibold border mb-4 ${
                status.success
        ...
        <div className="md:col-span-5 flex flex-col gap-6 justify-between">
          ...
          <div className="bg-[#111827]/40 border border-pink-500/10 rounded-3xl p-6 md:p-8 shadow-xl space-y-5">
            ...
            <p className="text-xs text-gray-400 leading-relaxed">
              Open my resume in a new tab to learn more about my technical
              skills, education, projects, and professional experience.
            </p>
